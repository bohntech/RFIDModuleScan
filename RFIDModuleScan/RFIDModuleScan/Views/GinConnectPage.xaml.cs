...
{
    public partial class GinConnectPage : ContentPage
    {
        ZXingScannerView zxing;
        Button torchButton = null;
        GinConnectPageViewModel vm = null;
        
        double width = 0;
        double height = 0;

        public GinConnectPage()
        {
            InitializeComponent();

            vm = new GinConnectPageViewModel(SimpleIoc.Default.GetInstance<INavigationService>(), SimpleIoc.Default.GetInstance<IModuleDataService>());
        ...
        private void initPage()
        {
            ...
            textOverlay.Text = "Hold device up to QR code to scan.";
        ...
        private void Zxing_OnScanResult(ZXing.Result result)
        {
            Device.BeginInvokeOnMainThread(() => {
                ...
                if (vm.IsValidConnectionCode(result.Text))
                {
                    vm.SaveConnectionInfo(result.Text);
                }
                else
                {
                    DisplayAlert("Invalid code", "QR code is not a valid connection code.", "OK");
                }
