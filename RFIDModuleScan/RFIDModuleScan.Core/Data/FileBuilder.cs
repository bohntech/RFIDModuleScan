...
namespace RFIDModuleScan.Core.Data
{
    public class FileHelper
    {
            ...
            foreach (var scan in scans)
            {
                ...
                foreach (var load in loads)
                {
                    ...
                    foreach (var module in moduleScans)
                    {
                        ...
                        dataString = dataString.Replace("{FieldID}", EscapeForCSV(scan.FieldID));

                        dataString = dataString.Replace("{LocalDate}", module.TimeStamp.ToString("yyyy/MM/dd"));
                        dataString = dataString.Replace("{LocalDate}", module.TimeStamp.ToString("yyyy/MM/dd"));
                        sb.AppendLine(dataString + "<CRLF>");
                        scanCount++;
        ...
        public static string GetCSVFileString(List<FieldScan> scans, IModuleDataService dataService)
        {
            ...
            //write header
            sb.AppendLine("Grower,Farm,Field,SerialNumber,ModuleID,Load,GinTicketLoadNumber,ScanLocation,ScanType,Timestamp,Latitude,Longitude,TabletID,Notes,GrowerID,FarmID,FieldID<CRLF>");
            ...
            foreach (var scan in scans) {
                ...
                foreach(var load in loads)
                {
                    ...
                    foreach(var module in moduleScans)
                    {
                        ...
                        sb.Append(EscapeForCSV(scan.GrowerID));
                        sb.Append(",");
                        sb.Append(EscapeForCSV(scan.FarmID));
                        sb.Append(",");
                        sb.Append(EscapeForCSV(scan.FieldID));
